...
import { bindNode, attachBindings } from './Bind.js';
...
import BindingDefinitions from './BindingDefinitions';
...
export const createTemplateInstance = function({ template, scope = {}, application = null, parentScope }) {
    ...
    if (typeof template === 'string') {
        visitedRefs.push(template.replace('#', ''));
    } else {
        visitedRefs.push(template.id);
    }
    ...
    while (template.hasAttribute('ref')) {
        const ref = template.getAttribute('ref');

        if (visitedRefs.indexOf(ref) > -1) {
            const stack = visitedRefs.reverse().reduce((carry, next) => {
                if (carry.length === 0) {
                    return `---> ${next}`;
                } else {
                    return `${carry}\n     ${next}`;
            ...
            console.error(`Circular template reference detected: \n${stack}`);
            return;
        }

        visitedRefs.push(ref);
    ...
    if (template.hasAttribute('src') && !template.processed) {
        return fetchExternalTemplate({ template: template, parentScope: parentScope })
            .then(template => createTemplateInstance({ template, scope, application, parentScope }));
    }

    scope = ({ view: scope, isIsolated: true, __proto__: ScopePrototype }).constructor();

    const node = instantiateTemplate(template, scope);

    if (isReplace) {
        const parentNode = targetTemplate.parentNode;
        const elementList = [].slice.apply(node.childNodes);
        ...
        parentNode.replaceChild(node, targetTemplate);
    }

    return { node : node, scope : scope };
};

export const instantiateTemplate = function(template, scope) {
    const node = document.importNode(template.content, true);
    const registeredBindings = BindingDefinitions.get(template.id);

    if (registeredBindings) {
        attachBindings(scope, node, registeredBindings);
    } else {

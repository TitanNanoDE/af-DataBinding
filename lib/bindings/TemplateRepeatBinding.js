...
const notifyRemoval = function(node) {
    const promises = [];

    if (node.preDisconnectCallback) {
        promises.push(node.preDisconnectCallback());
    }

    node.classList.add('removing');

    if (node.getAnimations) {
        promises.push(...node.getAnimations().map(animation => animation.finished));
    }
...
const TemplateRepeatBinding = {
    ...
    renderItemStable(model, scope, itemName, marker, item, index) {
        ...
        if (this.itemNodeList.has(item)) {
            ...
            const nodeIndex = nodeList.indexOf(node) - markerIndex;
            ...
            if (!marker.parentNode.children[markerIndex + index]) {
                marker.parentNode.appendChild(node);
            }

            marker.parentNode.insertBefore(node, marker.parentNode.children[markerIndex + index]);
        } else {
            let childScope = Make({
                $first : index === 0,

...
import { ObjectParser, parseExpression, parseAttributeName } from './Parser.js';
...
let checkNode = function(node, scope) {
    ...
    while (stack.length) {
        ...
        const parentNode = node.parentNode || node.ownerElement;
        ...
        if (BindingRegistry.get(attributeName) && BindingRegistry.get(attributeName).test()) {
...
const isBindingNode = function(node) {
    return node.isBindingNode || Object.getPrototypeOf(node) === Attr.prototype || node.nodeName === '#text';
};

 ...
 * @return {void}
 */
export let checkNode = function(node, scope, parentNode) {
    const dataRegex = /{{[^{}]*}}/g;
    ...
    if (node.nodeName == '#text' || isBindingNode(node)) {
        ...
        const testParameters = {
            ...
            singleExpression: singleBinding,
        };
        ...
        } else if (BindingRegistry.get(attributeName) && BindingRegistry.get(attributeName).test(testParameters)) {
            Make(BindingRegistry.get(attributeName))(bindingData);
        } else if (variables || singleBinding) {
            Make(BindingRegistry.get('#text'))(bindingData);
        }
        ...
        node.childNodes.forEach((childNode) => checkNode(childNode, scope, node));
    }
};
...
export const registerBindings = function(templateOrId, bindingDefinitions) {
    BindingDefinitions.register(templateOrId, bindingDefinitions);
};
